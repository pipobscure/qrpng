import { unsigned as crc32 } from './crc32.ts';
...
export async function bitmap(pixels: boolean[][]) {
	const { width, height, data } = await compileData(pixels);
	...
	setUint32(
		result,
		baseOffset + HEADER.length - 4,
		crc32(new Uint8Array(result, baseOffset + 4, HEADER.length - 8)),
	);
	...
	setUint32(
		result,
		baseOffset + DATA.length + data.byteLength,
		crc32(
			new Uint8Array(
				result,
				baseOffset + 4,
				DATA.length + data.byteLength + 4 - 8,
			),
		),
	);
	...
	setUint32(
		result,
		baseOffset + FOOTER.length - 4,
		crc32(new Uint8Array(result, baseOffset + 4, FOOTER.length - 8)),
	);
...
async function compileData(data: boolean[][]): Promise<{
	width: number;
	height: number;
	data: Uint8Array;
}> {
	const height = Math.ceil(data.length / 8) * 8;
	const width = Math.ceil((data[0]?.length ?? 0) / 8) * 8;
	...
	for (let line = 0; line < height; line++) {
		...
		for (let off = 0; off < width; off += 8) {
			...
			const byte =
				(!linedata[off + 0] ? 0b10_00_00_00 : 0) |
				(!linedata[off + 1] ? 0b01_00_00_00 : 0) |
				(!linedata[off + 2] ? 0b00_10_00_00 : 0) |
				(!linedata[off + 3] ? 0b00_01_00_00 : 0) |
				(!linedata[off + 4] ? 0b00_00_10_00 : 0) |
				(!linedata[off + 5] ? 0b00_00_01_00 : 0) |
				(!linedata[off + 6] ? 0b00_00_00_10 : 0) |
				(!linedata[off + 7] ? 0b00_00_00_01 : 0);
			buffer[byteOff] = byte;
			byteOff++;
		}
	}

	const result = await deflate(buffer);
	return { width, height, data: result };
}

async function deflate(buffer: Uint8Array) {
	const stream = new CompressionStream('deflate');
	const writer = stream.writable.getWriter();
	await writer.write(buffer as BufferSource);
	await writer.close();
	const buffers = await Array.fromAsync(stream.readable.values());
